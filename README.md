...
- **v2 + UI**: Research Portal — a Streamlit web interface for interactive querying, history tracking, evidence export, and evaluation dashboards
...
### Option 3: Research Portal UI (v2 + Streamlit)

The Research Portal is a browser-based interface built on top of the v2 pipeline. It provides an interactive way to search documents, review history, export evidence tables, and view evaluation metrics — no terminal required after setup.

```powershell
# One-time setup (if not already done)
.\run_setup.bat        # Runs setup_v2_RAG_UI.ps1 — installs dependencies and builds the index

# Launch the portal
.\run_portal.bat       # Opens the Streamlit app in your browser
```

> **Note:** `run_setup.bat` must be run before `run_portal.bat`. After setup completes, you only need `run_portal.bat` for subsequent sessions.
...
The portal (`app.py`) is a Streamlit application with three pages accessible from the sidebar:

| Page | Description |
|------|-------------|
| **Search** | Ask questions, view answers with cited sources, export evidence tables |
| **History** | Review all questions and answers from the current session |
| **Evaluation** | View confidence metrics, groundedness, precision/recall across all queries |

### Search Page

- Enter a question and click **Search**
- If relevant chunks are found (similarity ≥ 0.35), the LLM generates a cited answer
- Retrieved source chunks are shown in expandable sections, marked ✅ if cited in the answer
- Click **📥 Generate Evidence Table** to extract all claims and their supporting evidence
  - Claims with citations are matched to their source chunk and similarity score
  - Claims without citations are flagged with a warning
  - Download the full table as a CSV

### History Page

- Displays all questions asked during the current session
- Shows the full answer and number of sources cited for each query

### Evaluation Page

Displays quality metrics for all queries in the current session:

| Metric | Description |
|--------|-------------|
| **Groundedness** | % of answer sentences that have citations |
| **Citation Precision** | % of citations that refer to valid retrieved chunks |
| **Citation Recall** | % of retrieved chunks that were actually cited |
| **Evidence Strength** | Mean similarity score of cited chunks |
| **Retrieval Coverage** | Max similarity score across retrieved chunks |
| **Overall Confidence** | Weighted composite score (0.0–1.0) |

The page also highlights the highest and lowest confidence answers from the session.

---

## Setup Scripts

### `setup_v2_RAG_UI.ps1` (via `run_setup.bat`)

**What it does:**
1. ✅ Sets PowerShell execution policy if needed
2. ✅ Verifies Python 3.12 is installed
3. ✅ Creates project directories (`data/`, `index/`, `logs/`)
4. ✅ Creates a virtual environment (`.venv`)
5. ✅ Installs dependencies from `requirements.txt`
6. ✅ Verifies FAISS installation
7. ✅ Runs `run_all` (ingests docs, creates chunks, builds FAISS index)
...
Same steps as above but for the terminal-only versions. See Option 1 and Option 2 above.
...
- To re-test questions without rebuilding the index
...
Once found, PDFs were verified and downloaded manually.
...
├── setup_v2_RAG_UI.ps1             # Full setup for v2 + Streamlit UI
├── run_setup.bat                   # Launches setup_v2_RAG_UI.ps1
├── run_portal.bat                  # Launches the Streamlit Research Portal
├── app.py                          # Streamlit UI (Search, History, Evaluation)
...
| **First time setup (UI)** | `.\run_setup.bat` |
| **Launch Research Portal** | `.\run_portal.bat` |
...
**"Pipeline index not found"**: Run `run_setup.bat` or `setup_vX.ps1` first

**"LLM error"**: Verify Ollama is running (`ollama list`)

**Scripts won't run**: Both setup scripts handle execution policy automatically

**Portal won't open**: Ensure the venv is active and `streamlit` is installed. Try running `streamlit run app.py` manually after activating the venv.
