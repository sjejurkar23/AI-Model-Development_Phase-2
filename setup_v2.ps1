...
# -----------------------------
# 7. Build RAG Pipeline Index (v2)
# -----------------------------
Write-Host "`n=== Building RAG Pipeline v2 (ingest + chunk + index) ===" -ForegroundColor Cyan

python rag_pipeline_eval_v2.py run_all

if ($LASTEXITCODE -ne 0) {
    Write-Host "ERROR: Pipeline v2 build failed." -ForegroundColor Red
    exit 1
}

Write-Host "Pipeline v2 index built successfully." -ForegroundColor Green

# -----------------------------
# 8. Run Batch Evaluation (v2)
# -----------------------------
Write-Host "`n=== Running Batch Evaluation Questions (v2) ===" -ForegroundColor Cyan
...
Write-Host "Batch evaluation v2 complete." -ForegroundColor Green

# -----------------------------
# 9. Done
# -----------------------------
Write-Host "`n=== Setup Complete (v2)! ===" -ForegroundColor Cyan
Write-Host "Pipeline v2 is ready. You can now:" -ForegroundColor Cyan
Write-Host "  - Query: python rag_pipeline_eval_v2.py query --question 'Your question here'" -ForegroundColor White
Write-Host "  - Re-run batch: python rag_pipeline_eval_v2.py batch" -ForegroundColor White
Write-Host "  - Check logs: logs/rag_logs.jsonl and logs/eval_results.jsonl" -ForegroundColor White
Write-Host "`nv2 Features: Citation recall, confidence scoring, and quality gating enabled" -ForegroundColor Yellow
